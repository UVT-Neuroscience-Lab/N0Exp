...
public class PlayerController : MonoBehaviour
{
    ...
    public bool hasKey = false, hasFinalKey = false, pulledLever = false; 
    ...
    public void Interact()
    {
        ...
        if (interactObject.CompareTag("Door"))
        {
            ...
            if(hasKey && !door.isFinalDoor)
            {
                door.UnlockDoor();
            }
            else
            {
                if (hasFinalKey && door.isFinalDoor && pulledLever)
                {
                    door.UnlockDoor();
                }
            }
        ...
        else if (interactObject.CompareTag("Key"))
        {
            ...
            else
            {
                if(!hasFinalKey)
                {
                    var gfx = interactObject.parent.Find("GFX");
                    if (gfx) gfx.GetComponent<Movable>().move = true;
                    Invoke(nameof(TakeFinalKey), 0.5f);
                }
            }
        }
        else if (interactObject.CompareTag("Lever"))
        {
            if(!pulledLever)
            {
                pulledLever = true;
